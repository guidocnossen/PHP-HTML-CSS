...
<?php 
 ...
 if (mysql_num_rows($result) > 0) {
 		while($row = mysql_fetch_assoc($result)) {
    		array_push($_SESSION['Vragen'], $row['q_number'] = array('Vraag' => $row['q_text'], 
    ...
    if (isset($_POST['Antwoord'])){
        ...
        foreach ($_SESSION['Vragen'] as $Vraagnr => $Value){
            // Echo the question
            echo $Value['Vraag'].'<br />';
            if ($Antwoord[$Vraagnr] != $Value['CorrectAnswer']){
                ...
                echo '<span style="color: green;">1 punt</span>'; 
    				...
    				echo '<h3>Klik <a href="sessionquiz.php">hier</a> om de quiz opnieuw te beginnen!</h3>';
    } else {
?>
    <form action="sessionquiz.php" method="post" id="quiz">
		
    		<?php foreach ($_SESSION['Vragen'] as $Vraagnr => $Value){?> 
        <li>
